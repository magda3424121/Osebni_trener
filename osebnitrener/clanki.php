...
$id_vu = $_SESSION['id_vu'] ?? 0;  // admin=1
$id_u = $_SESSION['idu'] ?? 0;
...
<?php while ($clanek = mysqli_fetch_assoc($result)): ?>
    <article>
        ...
        <?php if ($id_vu == 1): ?>
            <p>
                <a href="uredi_clanek.php?id_c=<?= $clanek['id_c'] ?>">Uredi</a> | 
                <a href="izbrisi_clanek.php?id_c=<?= $clanek['id_c'] ?>" onclick="return confirm('Res želite izbrisati ta članek?')">Izbriši</a>
            </p>
        <?php endif; ?>
        ...
        <?php if (mysqli_num_rows($kom_result) > 0): ?>
            <?php while ($kom = mysqli_fetch_assoc($kom_result)): ?>
                <div class="komentar">
                    <strong><?= htmlspecialchars($kom['ime'] . ' ' . $kom['priimek']) ?>:</strong><br>
                    <?= nl2br(htmlspecialchars($kom['opis'])) ?>
                </div>
            <?php endwhile; ?>
        <?php else: ?>
            <p>Ni komentarjev.</p>
